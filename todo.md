...
| 维度 | 内容 |
|---|---|
| **为何做** | 服务端宕机恢复后，如果用户不刷新页面也不触发新数据，IndexedDB 中的历史数据永远不会被补发。需要一种不需要用户操作的自动检测机制。 |
| **如何做** | 抛弃 `window.online` 事件（localhost 不触发 + 只反映 OS 网络状态），改用 `setInterval` 周期轮询：每 10 秒从 DB 取数据 → fetch 尝试发送 → 成功则 `clearDB` + 停止轮询；失败则继续等待下次。`retryTimerId` 标记防止重复启动。 |
| **效果** | 服务端恢复后最多 10 秒延迟自动补发，无需页面刷新或用户操作。日志覆盖所有关键节点（💾写入 / 🔄启动轮询 / 🔍检查 / ✅成功 / 🛑停止）。 |
...
| 服务端重联自动补发（周期轮询） | 1 | +30 | ✅ 完成 |
| **P1 可视化面板** | **1** | **~100** | 🔲 待做 |
| **P2 长任务 + 采样** | **2** | **~80** | 🔲 待做 |
| **P3 README** | **1** | **N/A** | ✅ 完成 |

**当前已写：~510 行 | 全量预估：~690 行**
